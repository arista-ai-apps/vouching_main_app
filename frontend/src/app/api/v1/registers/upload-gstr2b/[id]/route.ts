...
// POST /api/v1/registers/upload-gstr2b/[id]
export async function POST(
  ...
  try {
    ...
    console.log(`[REGISTERS] GSTR-2B: parsed ${rawRows.length} rows from ${file.name}`);
    ...
    return NextResponse.json({ id: register.id, type: 'gstr2b', rows: register._count.rows }, { status: 201 });
  } catch (error) {
    console.error('[REGISTERS] GSTR-2B upload error:', error);
    return NextResponse.json({ error: 'Failed to upload GSTR-2B register' }, { status: 500 });
  }
}

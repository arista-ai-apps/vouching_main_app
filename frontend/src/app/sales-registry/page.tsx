...
import { FileSpreadsheet, Info, Loader2, AlertCircle, FileText, CheckCircle } from 'lucide-react';
...
export default function SalesRegistry() {
  ...
  const fetchRegistry = async () => {
    ...
    try {
      let rowsData: any[] = [];
      const rowsResp = await fetch(`${API_BASE}/registers/sales/${engagementId}/rows`);
      if (rowsResp.ok) {
        rowsData = await rowsResp.json();
        setRows(rowsData);
      }
      const regResp = await fetch(`${API_BASE}/registers/sales/${engagementId}`);
      if (regResp.ok) {
        const regData = await regResp.json();
        if (regData && regData.length > 0) {
          setRegisterInfo({ id: regData[0].id, filename: regData[0].filename, rows: rowsData.length });
        }
      }
    } catch (err) {
      console.error("Failed to fetch sales registry:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/registers/upload-sales/${engagementId}`, {
        method: 'POST',
        body: formData,
      });
      if (resp.ok) {
        ...
        setError(errData.detail || "Failed to upload sales register");
      }
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Sales Registry</h1>
          <p className="text-slate-500">Upload and view your Sales register Excel data.</p>
        </div>
        ...
        {loading ? (
          ...
          >
            <input
              ...
              className="hidden"
            />
            <div className={`p-4 rounded-full text-white mb-4 shadow-lg ${uploading ? 'bg-emerald-600' : 'bg-slate-800'}`}>
              {uploading ? <Loader2 size={32} className="animate-spin" /> : <FileSpreadsheet size={32} />}
            </div>
            <h3 className="text-lg font-bold text-slate-900">
              {uploading ? 'Analyzing Sales Register...' : 'Upload Sales Registry File'}
            </h3>
            <p className="text-sm text-slate-500 max-w-sm mt-2">
              Select your sales register Excel file. Arista AI will automatically map the fields.
            </p>
            {!uploading && (
            ...
            <p className="mt-8 text-[10px] text-slate-400 uppercase font-black flex items-center gap-2">
              <Info size={12} />
              Required columns: Sale No, Date, Buyer Name, Taxable, Total
            </p>
          </div>
        ) : (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="audit-card bg-emerald-50 border-emerald-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-emerald-800">{registerInfo?.filename || 'Sales Register Uploaded'}</p>
            ...
            <div className="bg-white rounded-xl shadow-xl shadow-slate-100 border border-slate-200 overflow-hidden">
              ...
              <div className="overflow-x-auto max-h-[600px] overflow-y-auto w-full relative">
                <table className="w-full text-[11px] whitespace-nowrap">
                  ...
                  <tbody>
                    {rows.map((row, i) => (
                      <tr key={row.id} className="hover:bg-emerald-50/30 border-b border-slate-50 last:border-0 transition-colors">
